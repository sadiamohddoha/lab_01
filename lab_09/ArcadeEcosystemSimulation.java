...
public class ArcadeEcosystemSimulation extends JFrame {
        ...
        public SetupPanel() {
            ...
            playButton.addActionListener(new ActionListener() {
                public void actionPerformed(ActionEvent e) {
                    ...
                    simContainer = new SimulationContainer(initialRabbits, initialWolves, carrotMax, carrotRefillInterval);
    ...
    // ---------------- SimulationContainer ------------------
    // Contains the simulation area and two graph panels (for rabbits and wolves).
    class SimulationContainer extends JPanel {
        ...
        public SimulationContainer(int initRabbits, int initWolves, int carrotMax, int carrotInterval) {
    ...
    // ---------------- SimulationArea ------------------
    // The area where carrots, rabbits, and wolves are drawn and updated.
    class SimulationArea extends JPanel {
        ...
        public SimulationArea(int initRabbits, int initWolves, int carrotMax, int carrotRefillInterval) {
        ...
        public void updateSimulation(int tick) {
            ...
            // Update wolves: move with direction and check for rabbit consumption.
            ...
            while (wit.hasNext()) {
                ...
                // Check if a wolf overlaps any rabbit.
                for (Rabbit r : new ArrayList<>(rabbits)) {
                    if (w.intersects(r)) {
                        rabbitsToRemove.add(r);
                        double logisticFactor = 1 - ((double) wolves.size() / wolfCarryingCapacity);
                        int offspring = (int) (wolfFecundity * logisticFactor);
                        for (int j = 0; j < offspring; j++) {
                            newWolves.add(new Wolf(w.x, w.y));
                        }
                        break; // one rabbit per update per wolf.
                    }
                }
            }
        ...
        protected void paintComponent(Graphics g) {
            ...
            // Draw wolves as dark gray circles.
            g.setColor(Color.DARK_GRAY);
            for (Wolf w : wolves) {
                g.fillOval((int)w.x, (int)w.y, w.size, w.size);
            }
        }
        ...
        // ----- Wolf with directional movement -----
        class Wolf {
            double x, y;
            int size = 20;
            int age = 0;
            int lifespan = 600;  // ticks
            double dx, dy;
            ...
            public void move() {
                ...
                if (rand.nextDouble() < 0.05) {
                    double anglePerturb = (rand.nextDouble() - 0.5) * (Math.PI / 4);
                    double currentSpeed = Math.sqrt(dx * dx + dy * dy);
                    double currentAngle = Math.atan2(dy, dx);
                    currentAngle += anglePerturb;
                    dx = currentSpeed * Math.cos(currentAngle);
            ...
            public boolean intersects(Rabbit r) {
                Rectangle wRect = new Rectangle((int)x, (int)y, size, size);
                Rectangle rRect = new Rectangle((int)r.x, (int)r.y, r.size, r.size);
                return wRect.intersects(rRect);
            }
        }
    }
    ...
    class GraphPanel extends JPanel {
        ...
        protected void paintComponent(Graphics g) {
            ...
            int maxPop = species.equals("Rabbits") ? rabbitCarryingCapacity : wolfCarryingCapacity;
    ...
    public static void main(String[] args) {
        SwingUtilities.invokeLater(() -> new ArcadeEcosystemSimulation());
    }
}
